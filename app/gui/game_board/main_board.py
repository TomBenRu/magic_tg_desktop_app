...
# Splitter für Handkarten und Spielfeld
player_splitter = QSplitter(Qt.Horizontal)
zone_layout.addWidget(player_splitter)

# Linke Seite: Spielerinfo, Bibliothek, Friedhof, Exil
left_widget = QWidget()
left_layout = QVBoxLayout(left_widget)

# Scrollbarer Bereich für den Friedhof
graveyard_group = QGroupBox(f\"Friedhof - Spieler {player_id}\")
graveyard_layout = QVBoxLayout(graveyard_group)
graveyard_scroll = QScrollArea()
graveyard_scroll.setWidgetResizable(True)
graveyard_content = QWidget()
self.graveyard_layouts = {} if not hasattr(self, 'graveyard_layouts') else self.graveyard_layouts
self.graveyard_layouts[player_id] = QVBoxLayout(graveyard_content)
graveyard_scroll.setWidget(graveyard_content)
graveyard_layout.addWidget(graveyard_scroll)
left_layout.addWidget(graveyard_group)

# Bibliothek
library_group = QGroupBox(f\"Bibliothek - Spieler {player_id}\")
library_layout = QVBoxLayout(library_group)
self.library_counts = {} if not hasattr(self, 'library_counts') else self.library_counts
self.library_counts[player_id] = QLabel(\"0 Karten\")
library_layout.addWidget(self.library_counts[player_id])
left_layout.addWidget(library_group)

# Exil
exile_group = QGroupBox(f\"Exil - Spieler {player_id}\")
exile_layout = QVBoxLayout(exile_group)
exile_scroll = QScrollArea()
exile_scroll.setWidgetResizable(True)
exile_content = QWidget()
self.exile_layouts = {} if not hasattr(self, 'exile_layouts') else self.exile_layouts
self.exile_layouts[player_id] = QVBoxLayout(exile_content)
exile_scroll.setWidget(exile_content)
exile_layout.addWidget(exile_scroll)
left_layout.addWidget(exile_group)

player_splitter.addWidget(left_widget)

# Mittlere Seite: Spielfeld
battlefield_group = QGroupBox(f\"Spielfeld - Spieler {player_id}\")
battlefield_layout = QVBoxLayout(battlefield_group)
battlefield_scroll = QScrollArea()
battlefield_scroll.setWidgetResizable(True)
battlefield_content = QWidget()
self.battlefield_layouts = {} if not hasattr(self, 'battlefield_layouts') else self.battlefield_layouts
self.battlefield_layouts[player_id] = QGridLayout(battlefield_content)
battlefield_scroll.setWidget(battlefield_content)
battlefield_layout.addWidget(battlefield_scroll)

player_splitter.addWidget(battlefield_group, 2)  # Mehr Platz für das Spielfeld

# Rechte Seite: Hand
hand_group = QGroupBox(f\"Hand - Spieler {player_id}\")
hand_layout = QVBoxLayout(hand_group)
hand_scroll = QScrollArea()
hand_scroll.setWidgetResizable(True)
hand_content = QWidget()
self.hand_layouts = {} if not hasattr(self, 'hand_layouts') else self.hand_layouts
self.hand_layouts[player_id] = QHBoxLayout(hand_content)
self.hand_layouts[player_id].setAlignment(Qt.AlignLeft)
hand_scroll.setWidget(hand_content)
hand_layout.addWidget(hand_scroll)

player_splitter.addWidget(hand_group)

# Größen für den Splitter einstellen
player_splitter.setSizes([200, 400, 200])
...
# Stapel
stack_group = QGroupBox(\"Stapel\")
stack_layout = QVBoxLayout(stack_group)
stack_scroll = QScrollArea()
stack_scroll.setWidgetResizable(True)
stack_content = QWidget()
self.stack_layout = QVBoxLayout(stack_content)
stack_scroll.setWidget(stack_content)
stack_layout.addWidget(stack_scroll)

middle_layout.addWidget(stack_group)

# Exil (gemeinsam)
shared_exile_group = QGroupBox(\"Gemeinsames Exil\")
shared_exile_layout = QVBoxLayout(shared_exile_group)
shared_exile_scroll = QScrollArea()
shared_exile_scroll.setWidgetResizable(True)
shared_exile_content = QWidget()
self.shared_exile_layout = QGridLayout(shared_exile_content)
shared_exile_scroll.setWidget(shared_exile_content)
shared_exile_layout.addWidget(shared_exile_scroll)

middle_layout.addWidget(shared_exile_group)

# Befehlszone
command_group = QGroupBox(\"Befehlszone\")
command_layout = QVBoxLayout(command_group)
command_scroll = QScrollArea()
command_scroll.setWidgetResizable(True)
command_content = QWidget()
self.command_layout = QGridLayout(command_content)
command_scroll.setWidget(command_content)
command_layout.addWidget(command_scroll)

middle_layout.addWidget(command_group)
...
`pathInProject`: `app / gui / game_board / main_board.py`
}